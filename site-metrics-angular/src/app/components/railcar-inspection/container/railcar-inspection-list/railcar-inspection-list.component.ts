...
import { TableService } from '../../data-grid/services/table.service';
import { DialogService } from '../../data-grid/services/dialog.service';
...
export class RailcarInspectionListComponent implements OnInit, OnDestroy {
  ...
  inspections$ = this.edit.inspections$;
  badOrders$ = this.edit.badOrders$;
  allBadOrders$ = this.edit.allBadOrders$;
  selectedRows$ = this.edit.selectedRows$;
  rowBackups$ = this.edit.rowBackups$;
  boRowBackups$ = this.edit.boRowBackups$;
  inspectionQueue$ = this.edit.inspectionQueue$;
  badOrderQueue$ = this.edit.badOrderQueue$;
  selectAll$ = this.edit.selectAll$;
  loading$ = this.edit.loading$;
  rowLoading$ = this.tableService.rowLoading$;
  deletedRows$ = this.edit.deletedRows$;
  ...
  cancelAllEdits(): void {
    this.dialogService
      .confirm(
        'Cancel All Edits',
        `You have ${this.queue.new.length + this.queue.modified.length} unsaved edit(s). Do you want to discard them?`
      )
      .subscribe(confirmed => {
        if (!confirmed) return;
  ...
  deleteSelectedRows(): void {
    if (!this.selectedRows || this.selectedRows.size === 0) {
      this.toast.show('No rows selected.', 'info');
      return;
    }
    this.dialogService
      .confirm(
        'Delete Selected Rows',
        `Are you sure you want to delete ${this.selectedRows.size} inspection(s) and their corresponding bad orders?`
      )
      .subscribe(confirmed => {
        if (!confirmed) return;
        this.tableService.setRowLoading(true);
        this.edit.deleteInspections(
          this.inspections,
          this.selectedRows,
          this.rowBackups,
          this.queue
        )
          .pipe(finalize(() => this.tableService.setRowLoading(false)))
          .subscribe({
            next: (deletedRows) => {
              ...
              this.paginationInspection.setData(this.inspections);
              this.loadDataForActiveTab();
              this.toast.show(`${deletedRows.length} inspections deleted successfully`, 'success');
            },
            error: () => {
  ...
  private loadInspections() {
    ...
    this.inboundService.getInspections()
      .pipe(
        ...
        finalize(() => this.tableService.setRowLoading(false))
      )
      .subscribe({
        next: data => {
  ...
  private loadBadOrders() {
    this.tableService.setRowLoading(true);
    this.badOrderService.getActiveBadOrders()
      .pipe(
        takeUntil(this.destroy$),
        finalize(() => this.tableService.setRowLoading(false))
      )
